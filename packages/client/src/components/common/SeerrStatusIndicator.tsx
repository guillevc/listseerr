...
interface SeerrStatusIndicatorProps {
  status: SeerrStatus;
  url?: string;
...
const statusConfig = {
  connected: {
    ...
    pulse: true,
    label: 'Connected',
    getDescription: (url?: string) => (url ? `${new URL(url).hostname}` : 'Connected to Seerr'),
  },
  error: {
    color: 'bg-red-500',
    pulse: false,
    label: 'Error',
    getDescription: () => 'Unable to connect to Seerr',
  },
  'not-configured': {
    color: 'bg-orange-400',
    pulse: false,
    label: 'Not Configured',
    getDescription: () => 'Seerr is not configured',
  },
  loading: {
    color: 'bg-gray-400',
    pulse: true,
    label: 'Checking',
    getDescription: () => 'Checking Seerr connection...',
  },
};

export function StatusDot({ status, className }: { status: SeerrStatus; className?: string }) {
...
export function SeerrStatusIndicator({
  status,
  url,
  pendingRequests,
  compact = false,
  children,
}: SeerrStatusIndicatorProps) {
  const config = statusConfig[status];
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            {children ?? (
              <span className="cursor-help" aria-label={`Seerr status: ${config.label}`}>
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            aria-label={`Seerr status: ${config.label}`}
          >
            <StatusDot status={status} />
          </button>
        </TooltipTrigger>
        <TooltipContent side="bottom">
          <div className="text-center">
            <p className="text-xs text-muted">Seerr</p>
            <p className="font-medium">{config.label}</p>
            <p className="text-xs text-muted">{config.getDescription(url)}</p>
          </div>
        </TooltipContent>
