...
export interface SeerrPendingRequestsResponse {
  pageInfo: {
    pages: number;
    pageSize: number;
    results: number;
 ...
 *
 * If mediaInfo is missing, the media is not in Seerr.
 */
export interface SeerrMediaInfoResponse {
  id: number;
  tmdbId: number;
  title?: string;
  mediaInfo?: {
